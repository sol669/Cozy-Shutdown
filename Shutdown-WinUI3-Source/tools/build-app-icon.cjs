// Deterministic preparation of the user-approved red image; no redrawing.
...
async function main() {
  ...
  // the letter S stays opaque, unlike a global white/gray color-key operation.
  ...
  for (let i = 0; i < count; i++) {
    ...
    // the white S and the shadow cast on the power symbol, retain their exact RGB.
  ...
  // Fill the icon canvas with the approved artwork, preserving its proportions.
  ...
  const normalized = await sharp(rgba, { raw: { width, height, channels: 4 } })
    .extract({ left, top, width: right - left + 1, height: bottom - top + 1 })
    .resize(976, 976, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 0 }, kernel: 'lanczos3' })
    .extend({ top: 24, bottom: 24, left: 24, right: 24, background: { r: 0, g: 0, b: 0, alpha: 0 } })
    .png().toBuffer();
  const source = () => sharp(normalized);
  console.log(`Artwork bounds ${right - left + 1}x${bottom - top + 1}; normalized to 95.3% canvas width.`);
  await source().png().toFile(path.join(output, 'ShutdownTrey.png'));
  ...
  fs.writeFileSync(path.join(output, 'ShutdownTrey.ico'), Buffer.concat([directory, ...frames]));
